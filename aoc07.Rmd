...
This works for the example problem but falls over on the large one:

```{r}
count_splits <- function(mat, c) {
  res <- 1

  if (nrow(mat) >= 2) {
    newmat <- mat[-1, , drop = FALSE]
  
    if (mat[1, c] == ".") {
      res <- count_splits(newmat, c)
    } else {
      res <- count_splits(newmat, c - 1) +
             count_splits(newmat, c + 1)
    }
  }
...
TBC. I have a DAG idea when I get the time.
