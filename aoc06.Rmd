...
To parse the numbers, I'll use *as.numeric*, which ignores whitespace either side of a number and returns NA when no number is present (that's the blank column, which we just skip over). Since the operator appears last when reading right to left, we can use its presence to trigger the sum.
